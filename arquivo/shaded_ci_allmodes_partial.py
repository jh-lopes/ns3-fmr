...
# ==========================================================
# Ajuste aqui os diretórios
# ==========================================================
RUN_DIR_BASE = Path("~/ns3-fmr/compare_runs/expA_fr1_100seeds_30s_baselines").expanduser()
RUN_DIR_FMR = Path("~/ns3-fmr/compare_runs/expA_fr1_100seeds_30s_fmr").expanduser()
...
# ==========================================================
# Escolhe diretório do modo
# ==========================================================
def get_run_dir(mode):
    if mode == "fmr_rl":
        return RUN_DIR_FMR
    return RUN_DIR_BASE
...
# Salvar CSV
# ==========================================================
csv_path = OUT_DIR / "ci95_ci99_allmodes_partial.csv"
...
def plot_shaded_ci(ci_col, ci_label, out_stem):
    ...
    for mode in MODES:
        ...
        plt.fill_between(
            ...
            upper,
            color=color,
            alpha=0.14 if mode != "fmr_rl" else 0.18,
            linewidth=0
        )
        ...
        # Anotar n apenas no FMR, porque ele ainda está parcial
        if mode == "fmr_rl":
            for x, y, n in zip(xs, means, ns):
                plt.annotate(
                    f"n={n}",
                    (x, y),
    ...
    plt.title(f"Vazão agregada média com {ci_label}\nBaselines completos e IA-FMR parcial")

    # ==========================================================
    # Mostrar exatamente todas as bandas simuladas no eixo X
    # ==========================================================
    ax = plt.gca()
    ax.xaxis.set_major_locator(FixedLocator(BANDS))
    ax.set_xticklabels([str(b) for b in BANDS])

    plt.xlim(min(BANDS) - 3, max(BANDS) + 3)
...
# Gerar figuras
# ==========================================================
plot_shaded_ci("ic95", "IC95%", "throughput_shaded_ci95_allmodes_partial")
plot_shaded_ci("ic99", "IC99%", "throughput_shaded_ci99_allmodes_partial")
...
print()
print("=" * 130)
print("RESUMO POR ESCALONADOR E BANDA")
print("=" * 130)

for r in rows:
    mode_label = LABELS[r["mode"]]
    if r["n"] == 0:
        print(f"{mode_label:>16} | BW={r['bandwidth_mhz']:>3} MHz | n=0")
    else:
        print(
            f"{mode_label:>16} | "
